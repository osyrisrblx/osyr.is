...
test('the page passes automated accessibility checks at desktop and mobile sizes', async ({
  ...
  for (const width of [1440, 390]) {
    ...
    const result = await new AxeBuilder({ page })
      ...
      .analyze();
    expect(result.violations, `${width}px accessibility violations`).toEqual(
      [],
    );
  }
});

test('touch can spin the head horizontally and scroll the page vertically', async ({
  browser,
  browserName,
}) => {
  test.skip(
    browserName !== 'chromium',
    'Touch gestures use Chromium’s DevTools input API.',
  );
  ...
  // The screenshot can scroll an element into view; place the gesture in the visible area.
  await canvas.scrollIntoViewIfNeeded();
  const bounds = (await canvas.boundingBox())!;
  const gestureX = Math.max(60, bounds.x + bounds.width * 0.35);
  const gestureY = Math.max(180, Math.min(500, bounds.y + bounds.height * 0.6));
  ...
  await gesture(gestureX, gestureY, gestureX + 110, gestureY);
  expect(await page.evaluate(() => scrollY)).toBe(scrollBefore);
  expect((await canvas.screenshot()).equals(before)).toBe(false);
  await canvas.scrollIntoViewIfNeeded();
  const verticalBounds = (await canvas.boundingBox())!;
  const verticalY = Math.max(
    280,
    Math.min(560, verticalBounds.y + verticalBounds.height * 0.6),
  );
  const verticalScrollBefore = await page.evaluate(() => scrollY);
  await gesture(190, verticalY, 190, verticalY - 200);
  await expect
    .poll(() => page.evaluate(() => scrollY))
    .toBeGreaterThan(verticalScrollBefore + 30);
  await context.close();
});
    ...
    for (const key of ['Home', 'ArrowUp', 'ArrowRight']) {
      ...
      const gaps = await page.evaluate(async (base64) => {
        ...
        return Array.from({ length: 21 }, (_, index) => {
          ...
          const [r, g, b] = context.getImageData(image.width - 3, y, 1, 1).data;
          return (
            Math.abs(r - 8) <= 1 && Math.abs(g - 8) <= 1 && Math.abs(b - 8) <= 1
          );
        }).filter(Boolean).length;
      }, screenshot.toString('base64'));
      expect(gaps, `${viewport.width}px, ${key}: uncovered right edge`).toBe(0);
    }
