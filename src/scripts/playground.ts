...
export async function initPlayground() {
	...
	const status =
		document.querySelector<HTMLParagraphElement>('#scene-status')!;
	...
	try {
		...
		await Promise.all([
			new OBJLoader()
				.loadAsync('/assets/biggerhead.obj')
				.then((loaded) => {
		...
		gradient = new THREE.DataTexture(
			new Uint8Array([
				75, 75, 75, 255, 165, 165, 165, 255, 255, 255, 255, 255,
			]),
			3,
			1,
			THREE.RGBAFormat,
		...
		function updateCursor() {
			canvas.dataset.interaction =
				pointerId !== undefined
					? 'drag'
					: hoverPosition &&
						  hitsHead(hoverPosition.x, hoverPosition.y)
						? 'hover'
						: 'none';
		}
		...
		function render(time: number) {
			...
			if (
				...
				for (let i = 0; i < steps; i++) {
					pitchVelocity +=
						((targetPitch - pitch) * 180 - pitchVelocity * 23) *
						step;
		...
		motionPreference.addEventListener(
			...
			() => {
				...
				cancelBlink();
				recordActivity();
				if (tracking && !motionPreference.matches)
					updateTrackingTarget();
				scheduleBlink();
				requestRender();
			},
